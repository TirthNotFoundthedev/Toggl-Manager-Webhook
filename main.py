...
@app.route('/login')
def auth_callback():
    if check_telegram_auth(request.args):
        session['user'] = request.args.to_dict()
        return redirect(url_for('dashboard'))
                ...
                if reply_text.startswith("🔔 Wake") and ":" in reply_text:
                    try:
                        ...
                        if len(parts_prompt) > 1:
                            ...
                            loading_msg = send_message(chat_id, f"🔔 Nudging {target_name}...")
                            loading_msg_id = loading_msg.get("result", {}).get("message_id") if loading_msg else None
            ...
            if command == "/start":
                ...
                if not supabase:
                    ...
                    if len(parts) > 1:
                        ...
                        loading_msg = send_message(chat_id, f"⏳ Processing status for {target_name}...")
                        loading_msg_id = loading_msg.get("result", {}).get("message_id") if loading_msg else None
                ...
                if not supabase:
                    ...
                    if target_name:
                        loading_msg = send_message(chat_id, f"⏳ Processing report for {target_name}...")
                        loading_msg_id = loading_msg.get("result", {}).get("message_id") if loading_msg else None
                ...
                if not supabase:
                    ...
                    loading_msg = send_message(chat_id, "⏳ Generating leaderboard...")
                    loading_msg_id = loading_msg.get("result", {}).get("message_id") if loading_msg else None
...
def telegram_webhook(request):
    ...
    Bridges the request to the Flask App.
    """
